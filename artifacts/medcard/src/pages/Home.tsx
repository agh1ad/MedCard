...
export function Library() {
  ...
  const currentFolder = library?.folders.find(
    (folder) => folder.id === folderId,
  );
  const currentNotebook = library?.notebooks.find(
  ...
  const visibleNotebooks =
    isSearching || notebookId
      ? []
      : (library?.notebooks ?? []).filter(
          (notebook) => (notebook.folderId ?? null) === folderId,
        );
  ...
  const folderTrail = buildFolderTrail(
    library?.folders ?? [],
      ...
      {libraryLoading || cardsLoading ? (
              ...
              <div className="library-collection-grid">
                ...
                {visibleNotebooks.map((notebook) => (
                  <NotebookTile
                    ...
                    count={
                      (cards ?? []).filter(
                        (card) => card.notebookId === notebook.id,
                      ).length
                    }
                  />
                ))}
          ...
          {visibleCards.length > 0 && (
            <section>
              <h2 className="library-section-title">
                {isSearching ? "Search results" : "Memory cards"}
              </h2>
        ...
        <div className="library-empty">
          <div className="library-empty-icon">
            {notebookId ? <BookOpen /> : <FolderIcon />}
          </div>
          <h2>
            {isSearching
              ? "No matching cards"
              : `This ${notebookId ? "notebook" : "folder"} is empty`}
          </h2>
          <p>
            {isSearching
  ...
  function FolderTile({ folder }: { folder: Folder }) {
    ...
    return (
      <article
        ...
        style={{ "--collection-color": folder.color } as React.CSSProperties}
      >
        <Link
          href={`/folders/${folder.id}`}
          className="library-collection-link"
        >
        ...
        <CollectionMenu
          ...
          onDelete={() => {
            if (
              !confirm(
                `Delete “${folder.name}”? Its contents will remain in Documents.`,
              )
            )
              return;
            deleteMutation.mutate(folder.id, {
              onSuccess: () => folderId === folder.id && setLocation("/"),
            });
          }}
        />
      </article>
  ...
  function NotebookTile({
    ...
    return (
      <article
        className="library-notebook-tile"
        style={{ "--collection-color": notebook.color } as React.CSSProperties}
      >
        <Link
          href={`/notebooks/${notebook.id}`}
          className="library-collection-link"
        >
          <span className="library-notebook-cover">
            <NotebookPen />
          </span>
          <span>
        ...
        <CollectionMenu
          ...
          onDelete={() => {
            if (
              !confirm(
                `Delete “${notebook.name}”? Its cards will return to Documents.`,
              )
            )
              return;
            deleteMutation.mutate(notebook.id, {
              onSuccess: () => notebookId === notebook.id && setLocation("/"),
            });
          }}
        />
      </article>
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button className="library-more" aria-label={`Actions for ${label}`}>
          <MoreHorizontal />
        </button>
      </DropdownMenuTrigger>
    ...
    updateMutation.mutate(
      { id: card.id, data: { notebookId: value ? Number(value) : null } },
      {
        onSuccess: () => {
          ...
          toast({
            title: value ? "Card moved to notebook" : "Card moved to Documents",
          });
        },
      },
    );
...
function CreateCollectionDialog({
  ...
  const submit = () => {
    ...
    mutation.mutate(input, {
      onSuccess: () => {
        toast({
          title: `${kind === "folder" ? "Folder" : "Notebook"} created`,
        });
        setName("");
        onClose();
      },
    ...
    <Dialog open={kind !== null} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            {kind === "folder"
              ? "Group notebooks and nested folders."
              : "Keep related MedCards together like pages in a notebook."}
          </DialogDescription>
        </DialogHeader>
        <Input
          value={name}
          ...
          placeholder={`${kind === "folder" ? "Folder" : "Notebook"} name`}
          autoFocus
        />
        ...
        <DialogFooter>
          ...
          <Button
            onClick={submit}
            disabled={
              !name.trim() || createFolder.isPending || createNotebook.isPending
            }
          >
            Create
          </Button>
